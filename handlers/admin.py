...
async def _send_with_retry(bot: Bot, chat_id: int, text: str, reply_markup=None, max_tries: int = 3) -> None:
    for attempt in range(max_tries):
        try:
            ...
            logger.warning("Flood control | chat=%s | wait=%ss | attempt=%s/%s", chat_id, e.retry_after, attempt + 1, max_tries)
...
async def fsm_prize_text(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Шаг 3/4 — <b>Сумма приза числом</b> (для статистики).\n"
...
async def _run_draw(msg: Message, session: AsyncSession, bot: Bot, edit: bool) -> None:
    ...
    await _notify_participants(bot, [p.telegram_id for p in all_parts],
...
async def _show_payments(msg: Message, session: AsyncSession, page: int, edit: bool) -> None:
    ...
    if not records:
        ...
        for r in records:
            ...
            lines.append(
                f"• <code>{r.telegram_id}</code> {name} {num}\n"
                f"  💛 {binance}  🎰 {stake}"
            )
        text = "\n".join(lines)
...
async def _group_new_contest(bot: Bot, contest) -> None:
    ...
    try:
        ...
        logger.info("Pin skipped (no rights) | %s", e)
...
async def _notify_participants(
    bot: Bot, participant_ids: list[int], winner_ids: set[int],
