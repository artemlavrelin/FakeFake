...
from config import ATM_INTRO, REPORT_CHANNEL_TITLE, REPORT_CHANNEL_URL
...
async def cmd_start(message: Message, session: AsyncSession, state: FSMContext) -> None:
    await state.clear()  # reset any active FSM
    ...
    if payload.startswith("contest_"):
        await _show_raffle(message, session, edit=False)
        return
...
async def cb_raffle(call: CallbackQuery, session: AsyncSession) -> None:
    await _show_raffle(call, session, edit=True)


async def _show_raffle(event, session: AsyncSession, edit: bool) -> None:
    ...
    if not contest:
        ...
        if edit:
            await msg.edit_text(text, parse_mode="HTML", reply_markup=back_to_menu_keyboard())
        else:
...
# ─── 🔥 Участвовать (кнопка на экране конкурса) ───────────────────────────────
...
async def cb_contest_participate(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    count  = await repository.get_participant_count(session, contest.id)
    # Show hypothetical chance if user joins
...
# ─── 🧲 Участвовать из группы (inline в группе) ───────────────────────────────
...
async def cb_group_join(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    await repository.add_participant(session, contest.id, call.from_user.id)
    count = await repository.get_participant_count(session, contest.id)
    await call.answer(f"✅ Вы зарегистрированы!\n👥 Участников: {count}", show_alert=True)
...
async def cb_report(call: CallbackQuery) -> None:
    ...
    await call.message.edit_text(
        text, parse_mode="HTML", reply_markup=report_keyboard(REPORT_CHANNEL_URL)
    )
    await call.answer()
...
async def cb_public_stats(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    await call.message.edit_text(
        format_public_stats(stats), parse_mode="HTML", reply_markup=public_stats_keyboard()
    )
    await call.answer()
...
async def cb_atm(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    has_data = pd is not None and (pd.binance_id or pd.stake_user)
    ...
    await call.message.edit_text(
        ...
        f"🎰 <b>Stake username:</b> {stake}",
        parse_mode="HTML",
        reply_markup=atm_main_keyboard(has_data),
    )
    await call.answer()
...
async def atm_binance(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🎰 <b>Введите ваш Stake username:</b>\n\n"
        "Если не используете — отправьте <code>-</code>",
        parse_mode="HTML",
        reply_markup=cancel_keyboard(),
    )
...
async def atm_stake(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"🎰 Stake username: <code>{stake}</code>",
        parse_mode="HTML",
        reply_markup=atm_confirm_keyboard(),
    )


@router.callback_query(ATMInput.confirm, F.data == "atm_save")
async def atm_save(call: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    data = await state.get_data()
    await state.clear()
    await repository.upsert_payment_data(
        session, call.from_user.id,
        binance_id=data.get("binance"),
        stake_user=data.get("stake"),
    )
    await call.message.edit_text(
        "✅ <b>Данные сохранены!</b>\n\n"
        "Ваши платёжные данные привязаны к аккаунту.",
        parse_mode="HTML",
        reply_markup=back_to_menu_keyboard(),
    )
